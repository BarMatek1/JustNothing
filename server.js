...
app.post("/dodaj", (req, res) => {
    ...
    const productsTable = "INSERT INTO produkty (nazwa_produktu, miesiac, ilosc, cena, suma, rok) VALUES (?, ?, ?, ?, ?, ?)";
    const expensesTable = "INSERT INTO koszty (nazwa_produktu, miesiac, ilosc, cena, suma, rok) VALUES (?, ?, ?, ?, ?, ?)";

    if(dbInfo === 'Product'){
        if (!nazwa || !miesiac || ilosc == null || cena == null || suma == null || rok == null ) {
            return res.status(400).json({ error: "Wszystkie pola są wymagane!" });
        }
    
        if (isNaN(ilosc) || isNaN(cena) || isNaN(suma) || ilosc <= 0 || cena <= 0 || suma <= 0 || rok <= 0) {
            return res.status(400).json({ error: "Ilość, cena i suma muszą być liczbami dodatnimi!" });
        }
